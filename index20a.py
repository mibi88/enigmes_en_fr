...
lvl = 1
def nondisp():
    showwarning("Info","Cette fonctionnalité est indisponible.")
def save():
    fichiercharg = asksaveasfile(title="Enregistrer le fichier de données",filetypes=[('Fichier MbGameStat','.mgs'),('all files','.*')])
    fichier = open(fichiercharg, "w")
    tmp_write = str(level)
    fichier.write("Bonjour monde")
    fichier.close()
def exeo():
    defi1.pack_forget()
    aidedefi.pack_forget()
def opens():
    filename = askopenfilename(title="Chargez le fichier de données",filetypes=[('Fichier MbGameStat','.mgs'),('all files','.*')])
    fichiero = open(filename, "r")
    lvlnum = fichiero.read()
    fichiero.close()
    if lvlnum == "1":
        restart()
    elif lvlnum == "2":
...
menu1.add_command(label="Ouvrir", command=opens)
menu1.add_separator()
menu1.add_command(label="Quitter", command=nondisp)
menubar.add_cascade(label="Fichier", menu=menu1)

menu2 = Menu(menubar, tearoff=0)
menu2.add_command(label="Recommencer", command=rstart)
menubar.add_cascade(label="Jeu", menu=menu2)
